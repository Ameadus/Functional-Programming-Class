 --FIRST ONE QUESTION LIST COMPREHESION 
 --2nd  QUESTION RECURSION 
 --LAST ONE OPEN ENDED 
--KNOW ISPRIME nextPrime fucntion KNOW FIBANOCHI KNOW PALINDROME KNOW FACTORIAL

--Maybe type may have a value or it might have nothing as a value 
--Nothing is a safe null 
--Just turns things into maybe types 
--Make sure to use Data.Maybe to use fromMaybe 


--Haskell partice from class 
...
--Collatz Sequences chaining 
chain :: (Integral a) => a -> [a]  
chain 1 = [1]  
chain n  
    | even n =  n:chain (n `div` 2)  
    | odd n  =  n:chain (n*3 + 1) 
--long chain lengths greater than 15 
numLongChains :: Int  
numLongChains = length (filter isLong (map chain [1..100]))  
    where isLong xs = length xs > 15 
--same thing with temp functions 
numLongChains :: Int  
